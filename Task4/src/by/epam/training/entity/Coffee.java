...
import by.epam.training.exception.CoffeeLogicalException;
...
public class Coffee implements Comparable<Coffee>, Cloneable {
	...
	public Coffee(String name, int pricePerGram, CoffeeState state) {
		try {
			if (name == null || name == "") {
				throw new CoffeeLogicalException("name");
			} else if (pricePerGram <= 0) {
				throw new CoffeeLogicalException("pricePerGram");
			} else {
				this.name = name;
				this.pricePerGram = pricePerGram;
				this.state = state;
			}
		} catch (CoffeeLogicalException e) {
